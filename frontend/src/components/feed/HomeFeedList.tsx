  ...
  const loadMore = async () => {
    ...
    try {
      const response = await fetch(
        `/api/feed/home?limit=${pageSize}&offset=${nextOffset}`,
        {
          method: "GET",
          cache: "no-store",
          credentials: "include",
      ...
      const payload = (await response
        .json()
        .catch(() => null)) as FeedPagePayload | null;
      if (!response.ok) {
        throw new Error(
          parseDetail(payload) ?? "Impossible de charger le fil.",
        );
      }
      ...
      console.error("Failed to load next feed page", loadError);
      setError(
